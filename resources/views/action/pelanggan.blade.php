<div>
   ...
   @if($pl->keterangan === 'member')
        ...
        <!-- Edit Button -->
        <button onclick="openEditModal({{ $pl->id_pelanggan }})" 
...
<!-- Modal for Edit -->
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 hidden z-50" onclick="closeOnOverlayClick(event, 'editModal')">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input 
                        type="text" 
                        name="nama_pelanggan"
                        value="{{ old('nama_pelanggan', $pl->nama_pelanggan) }}"
...
<!-- Modal for Delete -->
<div id="deleteModal" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50 hidden" onclick="closeOnOverlayClick(event, 'deleteModal')">
    <div class="bg-white p-6 rounded-lg shadow-lg w-96" onclick="event.stopPropagation()">
        ...
        <p class="mt-2 text-gray-600">Apakah Anda yakin ingin menghapus pelanggan <span id="deleteItemName"></span>? Tindakan ini tidak dapat dibatalkan.</p>
        <div class="mt-4 flex justify-end space-x-2">
            <form id="deleteForm" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none">Ya, Hapus</button>
            </form>
            <button onclick="closeDeleteModal()" class="px-4 py-2 bg-gray-300 text-black rounded-md hover:bg-gray-400 focus:outline-none">Batal</button>
        </div>
    </div>
</div>
...
<script>
    function openEditModal(id_pelanggan) {
        ...
        editForm.action = `/pelanggan/update/${id_pelanggan}`;
